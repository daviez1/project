...
export const createReservation = async (
  reservation: ReservationTypes.TableReservation
) => {
  try {
    await dbConnect();
    const reservations = await TableReservation.find();
...
export async function createReservationFromWaitlistEntry(
  entry: ReservationTypes.WaitlistEntry,
  table: ReservationTypes.Table
): Promise<ReservationTypes.TableReservation | null> {
  await dbConnect();
  const reservations = await TableReservation.find();
  const reservationId = String(reservations.length + 1);
